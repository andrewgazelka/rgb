...
struct DuneConfig {
    /// Base height of the terrain (Y level)
    base_height: i32,
    /// Maximum dune height above base
    max_dune_height: i32,
    /// Scale of the main dune ridges
    ridge_scale: f64,
    /// Direction of prevailing wind (radians)
    wind_angle: f64,
    /// How much the wind stretches dunes
    wind_stretch: f64,
}
...
fn get_dune_height(world_x: i32, world_z: i32, config: &DuneConfig) -> i32 {
    ...
    // Stretch along wind direction to create elongated dunes
...
pub fn create_dune_chunk(chunk_x: i32, chunk_z: i32) -> eyre::Result<Bytes> {
    ...
    // Heightmaps - empty map (varint 0 for map size)
    ...
    // Light Data - 4 BitSets + 2 lists of byte arrays
    ...
    // Empty sky light mask - sections 0-4 are empty (below Y 0)
    ...
    // Sky Light Arrays - list of byte arrays for sections with sky light
...
fn create_dune_sections(chunk_x: i32, chunk_z: i32) -> Vec<u8> {
    ...
    for section_y in 0..24 {
        let section_min_y = (section_y as i32 - 4) * 16; // Section 0 = Y -64, Section 4 = Y 0
